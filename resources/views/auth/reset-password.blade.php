...
@section('title', 'Réinitialiser le mot de passe - BiblioTech')
...
<div class="max-w-md mx-auto my-8 sm:my-16">
    <div class="glass-card rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        <h2 class="text-2xl font-outfit font-bold text-center mb-4 bg-gradient-to-r from-white to-indigo-200 bg-clip-text text-transparent">Nouveau mot de passe</h2>
        <p class="text-sm text-gray-400 text-center mb-6 leading-relaxed">
            Veuillez saisir votre nouveau mot de passe ci-dessous pour finaliser la réinitialisation de votre compte.
        </p>
        ...
        <form action="{{ route('password.update') }}" method="POST" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5">Adresse E-mail confirmée</label>
                <div class="px-4 py-2.5 rounded-lg bg-white/5 border border-white/5 text-gray-400 select-none font-medium text-sm">
                    {{ $email }}
                </div>
            </div>
            
            <div>
                <label for="password" class="block text-sm font-medium text-gray-400 mb-1.5">Nouveau mot de passe (min. 6 caractères)</label>
                <div class="relative">
                    <input type="password" id="password" name="password" class="w-full pl-4 pr-10 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/15 transition duration-200" required placeholder="••••••••" autofocus>
